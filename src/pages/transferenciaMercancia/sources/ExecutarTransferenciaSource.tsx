...
const ExecutarTransferenciaSource = () => {
  ...
  // Actualiza el total escaneado basado en el acumulado de la columna "Cantidad a Transferir".
  useEffect(() => {
    setTotalEscaneado(produtos.reduce((sum, p) => sum + p.quantidade, 0));
  }, [produtos]);
    ...
    try {
      ...
      if (data) {
        ...
        if (data.productos_ingreso?.length) {
          ...
          // Se calcula el total escaneado a partir de la suma de cada producto
  ...
  const handleBarcodeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Obtener id de la bodega seleccionado
        ...
        setProdutos(prev => {
          ...
          if (exists) {
            if (exists.quantidade + 1 > qtyAvailable) {
              ...
              // No se actualiza la cantidad si excede la cantidad disponible
              return prev;
            }
            return prev.map(p =>
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Consumir el endpoint de ingreso-products
    ...
    <div className="container mx-auto py-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <span className="text-sm text-gray-500">
          (Salida de productos Source): <strong>{transferencia.source_origen_name}</strong>
        </span>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => navigate('/dashboard/transferencia-sources')}>
            Regresar
          </Button>
          <Button
            onClick={handleSave}
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="mb-4">
              ...
              <div className="font-medium">
                {transferencia.fecha ? format(new Date(transferencia.fecha), "dd/MM/yyyy") : 'N/A'}
              </div>
            </div>
          </div>
        </div>
